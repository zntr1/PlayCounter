...
if (!gameName) throw new Error("Usage: tsx src/add-game.ts <game name>");
...
const response = await fetch("https://api.igdb.com/v4/games", {
  ...
  body: `search "${escapeIgdbString(gameName)}"; fields name,cover.image_id; limit 10;`,
});
...
const game =
  games.find(
    (candidate) => candidate.name.toLowerCase() === gameName.toLowerCase(),
  ) ?? games[0];
if (!game) throw new Error(`No IGDB game found for "${gameName}".`);
