  ...
  if (!journal.playthroughs.length)
    return compact ? null : (
      <Button
        variant="ghost"
        icon={BookOpen}
        onClick={() => open({ game: session, tab: "playthroughs" })}
      >
        Add a playthrough
      </Button>
    );
...
function GameJournalDialog({ target }: { target: JournalTarget }) {
  ...
  const gameSessions = useMemo(
    () => sessions.filter((s) => identity(s) === gameKey),
    [sessions, gameKey],
  );
        ...
        {tab !== "organize" ? (
          <>
            <label className="grid gap-2 text-sm text-text-muted">
              ...
              >
                <option value="">
                  {tab === "note" ? "General game note" : "Unassigned sessions"}
                </option>
                {journal.playthroughs.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.name}
              ...
              <div className="grid gap-4">
                ...
                <div className="rounded-lg border border-border bg-bg p-3 text-sm text-text-muted">
                  <span className="font-medium text-text">
                    ...
                    {journal.playthroughs.find(
                      (p) => p.id === journal.activePlaythroughId,
                    )?.name ?? "Unassigned"}
                  </span>
                  <p className="mt-1 text-xs">
                    New playthroughs become active for future sessions. Existing
                    sessions and imported playtime stay as they are.
                  </p>
                  {running.map((s) => (
                  ...
                  <section className="grid gap-3 rounded-xl border border-border p-4">
                      ...
                      <span className="font-mono text-lg font-semibold text-accent">
                        {formatDuration(
                          playthroughSeconds(
                            ...
                            running
                              .filter((s) => s.playthroughId === playthrough.id)
                              .reduce(
                                (sum, s) =>
                                  sum +
                                  Math.max(
                    ...
                    {deleting === playthrough.id ? (
                      <div className="rounded-lg border border-warning-border bg-warning-tint p-3 text-sm text-text">
                        ...
                        become unassigned and all game playtime is kept.
                ...
                <section className="grid gap-2">
                  <h3 className="text-sm font-semibold text-text">
                    {playthrough ? "Recorded sessions" : "Unassigned sessions"}{" "}
                    · {selectedSessions.length}
                  </h3>
                  {playthrough && (archived[playthrough.id] ?? 0) > 0 ? (
                    <p className="text-xs text-text-muted">
                      Total includes{" "}
                      {formatDuration(archived[playthrough.id], showDays)} from
                      archived sessions.
                    </p>
                  ) : null}
